...
void D3D11Renderer::DrawMotionTrail(ID3D11ShaderResourceView* currentSrv) {
    ...
    // Read from current accum (ping), write new accum to other texture (pong)
    ...
    // Step 1: render new accumulation to the trail texture (pong)
    {
        D3D11_VIEWPORT vp = {0,0,(float)width_,(float)height_,0,1};
        ctx_->RSSetViewports(1, &vp);
    ...
    // Step 2: blit trail output to backbuffer for display
    // Use a GPU-side draw instead of CopyResource to avoid sync stalls
    ID3D11ShaderResourceView* trailOutSrv = trail_ping_ == 0 ? trail_srv_b_ : trail_srv_a_;
    {
        D3D11_VIEWPORT vp = {0,0,(float)width_,(float)height_,0,1};
        ctx_->RSSetViewports(1, &vp);
        ctx_->OMSetRenderTargets(1, &rtv_, nullptr);

        // Temporarily override trail constants for passthrough: decay=0, opacity=1
        float identity[4] = {0.0f, 1.0f, 0.0f, 0.0f};
        D3D11_MAPPED_SUBRESOURCE m;
        if (SUCCEEDED(ctx_->Map(shaders_.trail_buffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &m))) {
            memcpy(m.pData, identity, sizeof(identity));
            ctx_->Unmap(shaders_.trail_buffer, 0);
        }
        ...
        ID3D11ShaderResourceView* srvs[2] = {trailOutSrv, trailOutSrv};
        ctx_->PSSetShaderResources(0, 2, srvs);
