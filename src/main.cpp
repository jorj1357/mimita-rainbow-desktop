...
static DWORD WINAPI OverlayThread(LPVOID param) {
    ...
    AppConfig cfg; ConfigApply(cfg); // start with defaults
    ...
    for (int r = 0; r < 120; r++) {
        ID3D11Texture2D* tex = capture.GetFrame();
        if (tex) {
            D3D11_SHADER_RESOURCE_VIEW_DESC sd = {};
            sd.Format = DXGI_FORMAT_B8G8R8A8_UNORM;
            sd.ViewDimension = D3D11_SRV_DIMENSION_TEXTURE2D;
            sd.Texture2D.MipLevels = 1;
            if (SUCCEEDED(renderer.Device()->CreateShaderResourceView(tex, &sd, &srv))) {
                Log::Write("first frame captured"); break;
            }
        }
        Sleep(16);
    }
    ...
    while (g_overlayRunning) {
        cfg = ConfigRead();  // thread-safe read from shared memory

        if (!cfg.enabled) { Sleep(16); continue; }

        capture.GetFrame();
        if (!srv) { Sleep(1); continue; }
